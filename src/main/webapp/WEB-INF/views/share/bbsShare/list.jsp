			...
			<table>
					<c:forEach items="${list}" var="vo" varStatus="status">
						...
						<td onclick="location.href='one?bbsShareNo=${vo.bbsShareNo}'">
							...
							<h2 class="title">${fn:length(vo.bbsShareTitle) > 10 ? fn:substring(vo.bbsShareTitle, 0, 10).concat('...') : vo.bbsShareTitle}</h2>
							<div class="flex-row">
			...
			<div class="row">
				<div class="col-12">
					<div class="load-more-btn text-center mt-70">
						<c:if test="${pageno > 1}">
							<a href="list?pageno=${pageno-1}" class="btn oneMusic-btn">이전</a>
						</c:if>
						<a href="list?pageno=${pageno+1}" class="btn oneMusic-btn">다음</a>
					</div>
				</div>
			</div>
		</div>
